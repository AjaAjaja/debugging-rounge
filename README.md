...
현재는 구글 OAuth 기반 로그인 + JWT만 사용하지만, 실제 서비스라면 여러가지 소셜 로그인 추가뿐만 아니라 추후에 RefreshToken 저장 방식이나 저장소 변경(Redis) 등 요구가 생길 수 있다고 봤다. 또한 나중에 질문/답변 알림을 이메일·카카오톡·푸시 등으로 확장할 때, Application 로직을 건드리지 않고 알림 어댑터만 교체/추가하는 구조를 만들고 싶었습니다. 

**그래서 이런 부분을 Port/Adapter로 분리하면 변경 범위를 최소화할 수 있다고 생각해서 헥사고날을 적용했다.**
...
- 코어 로직은 Port 인터페이스를 의존하고 있기 때문에
- 외부 인프라가 변경되어도 코어 로직의 코드 뿐만 아니라 코어 로직 테스트 코드 또한 변경이 거의 없음
