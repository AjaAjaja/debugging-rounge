...
- Facade: Facade는 여러 **In Port(유즈케이스)**를 처리하는 진입점으로, 한 곳에서 정책 일관성과 트랜잭션 경계를 유지하며 구현합니다. 반면 Service라는 이름은 도메인 서비스인지 애플리케이션 서비스인지 그 의미가 모호해지기 쉬워 설계 의도가 흐려질 수 있어 Facade를 사용(AuthFacade, QuestionFacade, UserFacade 등)
