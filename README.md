...
### 리프레쉬 토큰 전략
JWT의 한계와 단점을 보완하는데 관심을 집중

- **단일 토큰 원칙** - 사용자가 새로 로그인하거나 재발급을 받는다면 즉시 기존 Refresh Token을 폐기 → 동시에 여러 개의 토큰 생성 불가
- **재사용 감지 시 전량 폐기** - 폐기된 토큰 재사용이 감지가 되면 해당 유저의 모든 리프레쉬 토큰 폐기 → 이 후 다중 기기 정책 도입 시에도 일관된 제어가 가능
- **보안을 고려한 저장** - Refresh Token 원문을 저장하지 않고, 단방향 해시 값으로 저장 → 유출 시에도 원문 노출을 최소화
